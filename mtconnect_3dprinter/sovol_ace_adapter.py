...
def get_printer_data():
    """
    Fetches data from Moonraker API via HTTP. Note: when communicating with Unity digitial twin you 
    will need to allow HTTP because Unity defaults to only allow HTTPS
    """
    # Tells API Klipper to give us exactl data needed
    url = f"http://{TCP_IP}:{TCP_PORT}/printer/objects/query?print_stats&toolhead&extruder&heater_bed"
    try:
        with urllib.request.urlopen(url, timeout=2) as response:
            return json.loads(response.read().decode())
    except Exception as e:
        # If this shows in the terminal, the Pi can't reach the printer API
        # print(f"API Error: {e}") 
        return None
...
def main():
    ...
    # Define Data Items from XML documentation
    avail = Event('avail')
    ...
    try:
        while True:
            ...
            if data and 'result' in data:
                ...
                # Update Positions (Moonraker Path)
